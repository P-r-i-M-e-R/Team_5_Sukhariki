...
class Simulation:
    ...
    def run(self):
        ...
        for i, t in enumerate(t_arr):
            ...
            mu = self.system.m0 / m
            zd, zd_dot, zd_ddot = self.system.reference(
                t, p["z_step"], p["t_step"]
            )

            # --- backstepping ---
            z_bs, zdot_bs = bs_state[i]
            T_bs, e_bs, ve_bs, delta_bs = self.bs.compute(
                z_bs, zdot_bs, zd, zd_dot, zd_ddot, m
            )
            bs_T[i]     = T_bs
            bs_e[i]     = e_bs
            bs_v_e[i]   = ve_bs
        ...
        zd_arr = np.array(
            [self.system.reference(t, p["z_step"], p["t_step"])[0] for t in t_arr]
        )
