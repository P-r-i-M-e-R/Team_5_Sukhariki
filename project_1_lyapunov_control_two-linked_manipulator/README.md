...
```math
a = -k_1 e - k_2 \dot{\theta} + G(\theta) \qquad \text{(3)}
```
...
```math
M(\theta) \ddot{\theta} + C(\theta,\dot{\theta}) \dot{\theta} + k_2 \dot{\theta} + k_1 e = 0 \qquad \text{(4)}
```
...
```math
L(e, \dot{\theta}) = \underbrace{\frac{1}{2} \dot{\theta}^T M(\theta) \dot{\theta}}_{\text{Kinetic Energy}} + \underbrace{\frac{1}{2}k_1 e^T e}_{\text{Potential Energy}} \qquad \text{(5)}
```
...
```math
\dot{L} = -k_2\|\dot{\theta}\|^2 \qquad \text{(6)}
```
...
```math
e(t) \to 0 \quad \text{and} \quad \dot{\theta}(t) \to 0 \quad \text{as} \quad t \to \infty \qquad \text{(7)}
```
